...
public class AddUsersToStationUseCase {
    ...
    public Station execute(UUID stationId, Set<String> usersLogin) {
        ...
        Set<User> users = userRepository.findByLoginIn(usersLogin)
                .stream()
                .filter(not(User::hasRoleSuperAdminOrAdmin))
                .collect(toSet());
        if (users.isEmpty()) {
            throw new UserNotFoundException("Users " + usersLogin + " not found.");
        }
        users.forEach(station::addUser);
        userRepository.updateUsersStation(users.stream().map(User::getLogin).collect(toSet()), stationId);
        stationEventPublisher.publishUsersAddingToStation(new UserAddingToStationEvent(stationId, usersLogin));
        return station;
    }
}
